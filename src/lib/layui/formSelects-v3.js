...
(function(layui, window, factory) {
    ...
    var $ = layui.jquery || $,
            ...
            methods: {
                ...
                addLabel: function addLabel(options, vals) {
                    ...
                    vals.forEach(function(val) {
                        ...
                        var _ref2 = [$('<span  class="item"' + tips + ' value=\'' + val.val + '\'><em ' + tips + '>' + val.name + '</em></span>'), $('<i ' + tips + ' class="layui-icon">&#x1006;</i>')],
                            $label = _ref2[0],
                            $close = _ref2[1];
                ...
                typeInit: function typeInit(options, filter) {
                    ...
                    if (options.type == 2) {
                        ...
                        div.find('dl dd:not(.layui-select-tips)').each(function(index, target) {
                            ...
                            $target.text('');
                            $target.append('<span lay-filter="' + filter + '"><input type="checkbox" title="' + text + '" lay-skin="primary" ' + dis + '> </span>');
                        });
                ...
                typeHandler: function typeHandler(options, dd, isAdd) {
                    if (options.type == 3) {
                        //对勾
                        if (isAdd) {
                            var span = $('<span><i class="layui-icon ' + options.icon.class + '">' + options.icon.text + '</i></span>');
                            dd.append(span);
                        } else {
                            dd.find('span').remove();
                        }
                ...
                overrideInput: function overrideInput(options) {
                    ...
                    if (_dom.attr('disabled') != undefined) {
                        $orinput.append($('<div style="    width: 100%;    height: 100%;    display: block;    position: absolute;    left: 0;    top: 0;"></div>').addClass('layui-disabled'));
                    }
                },
        ...
        styles = {
            ...
            css: function css() {
                return 'select[xm-select] + div .layui-select-title > input{display: none!important;}';
            },
            cssColor: function cssColor(name, spanBgColor, spanColor, iBgColor, iColor, borderColor, tBgColor, tColor) {
                return 'select[xm-select][xm-select-skin=\'' + name + '\'] + div .layui-select-title > div.xm-select > span:not(.xm-select-empty){background: ' + spanBgColor + ';color: ' + spanColor + ';border: 1px solid ' + borderColor + '}select[xm-select][xm-select-skin=\'' + name + '\'] + div .layui-select-title > div.xm-select > span:not(.xm-select-empty) i{background: ' + iBgColor + ';color: ' + iColor + ';}select[xm-select][xm-select-skin=\'' + name + '\'] + div .layui-select-title > div.xm-select > span:not(.xm-select-empty) i:hover{opacity:.8;filter:alpha(opacity=80);cursor: pointer;}select[xm-select][xm-select-skin=\'' + name + '\'][xm-select-type=\'1\'] + div dl dd.xm-select-this:not(.layui-disabled),select[xm-select][xm-select-skin=\'' + name + '\']:not([xm-select-type]) + div dl dd.xm-select-this:not(.layui-disabled){background-color: ' + tBgColor + ';color: ' + tColor + ';}select[xm-select][xm-select-skin=\'' + name + '\'][xm-select-type=\'2\'] + div dl dd.xm-select-this:not(.layui-disabled) i{background-color: ' + tBgColor + ';border-color: ' + tBgColor + ';color: ' + tColor + ';}select[xm-select][xm-select-skin=\'' + name + '\'][xm-select-type=\'3\'] + div dl dd.xm-select-this:not(.layui-disabled){color: ' + tBgColor + ';}select[xm-select][xm-select-type=\'3\'] + div dl dd.xm-select-this i{position: absolute;right: 10px;}\n';
            },
            renderCss: function renderCss() {
                return 'select[xm-select] + div .layui-select-title > div.xm-select > span:not(.xm-select-empty){box-sizing: content-box;}.xm-select-parent .layui-form-select dl dd.layui-this{background-color: inherit;    color: inherit;}.xm-select-parent .layui-form-select dl dd.layui-this.layui-select-tips {background-color: #FFFFFF;    color: #999999;}.xm-select-parent .layui-form-selected dl {    display: none;}.xm-select-show:{display: block;}.xm-select-hidn:{display: none;}\n';
            }
        };
    commons.methods.run();
    return select3;
