...
(function (scope = {}) {
    ...
    scope.dataInit = function (params) {
        ...
        scope.getRealTime = function () {
            api.realTimeRegister(params).then(res => {
    ...
    scope.renderChart = (asynData) => {
        ...
        asynData.list.map((item,index) => {
            ...
            item.data.map(DataItem => { seriesData1.push(DataItem.total_user); seriesData2.push(DataItem.sub_sum_user) });
            if(index === 0 ){
    ...
    scope.renderTable = (asynData) => {
        //  表头文本
        let thead = { date: "日期", platform_name: "系统平台", channel_name: "常规渠道", server_name: "服务器", sum_user: "注册人数" };
            ...
            asynData.list.forEach(listItem => {
                listItem.data.forEach(dataItem => {
                    if (dataItem.window_end === item) {
                        listItem[`cur${curTime}`] = dataItem.total_user
                    }
                })
            })
        })
    ...
    scope.downloadExcel = (params, isExistData) => {
        ...
        $('#downExcel').unbind("click").on('click', function () {
            ...
            window.open('/basic_v2/real-time/register-flow?' + tempParams)
