...
(function (scope = {}) {
    ...
    scope.dataInit = function (params, nodes) {
        ...
        api.channelRigister(params).then(res => {
            _.ErrorMsg(res)
            ...
            //  表头文本
            let thead = ['日期', '系统平台', '常规渠道', '渠道新增账号数', '新增充值总额', '新增充值账号数', '充值ARPU', '新增总充值次数', '平均充值额/次', '付费渗透率', '新增消耗总额', '新增消耗账号数', '消耗ARPU', '新增总消耗次数', '平均消耗额/次', '消耗渗透率']
    ...
    scope.downloadExcel = (params, isExistData) => {
        ...
        $('#downExcel').unbind("click").on('click', function () {
            ...
            window.open('/basic/channel/channel-register?' + tempParams)
